...
% figure
% set(gcf, 'Position',  [100, 100, 1400, 160])
% 
% win=20;
% plot((t_bins3sec(1:length(LH_valid)))/3600,mov_avg_nan(LH_valid,win),'linewidth',1.0,'color',[.4 .4 .9]);
% xlim(t_plot/3600); ylim([0 1.1*max(mov_avg_nan(LH_valid,win))]); 
% hold on
...
% plot((t_bins3sec(ind))/3600,mov_avg_nan(LH_valid(ind),win),'linewidth',1.0,'color',[1 .4 .4]);
SWS_thresh=mov_avg_nan(LH_valid,win)+.4*mov_iqr_nan(LH_valid,win);
% plot((t_bins3sec(1:length(SWS_thresh)))/3600,SWS_thresh,'linewidth',1.0,'color',[1 .5 .5]);
REM_thresh=mov_avg_nan(LH_valid,win)-.4*mov_iqr_nan(LH_valid,win);
% plot((t_bins3sec(1:length(REM_thresh)))/3600,REM_thresh,'linewidth',1.0,'color',[1 .5 .5]);
...
% plot(REM_onsets(valid_REM_inds)/3600,LH_valid_smoo(REM_inds(valid_REM_inds)),'.','markersize',14,'linewidth',1.0,...
%     'color',.3*[.6 .6 .9]);
% valid_SWS_inds=SWS_onsets>light_off_t+20*60 & SWS_onsets<light_on_t;
% plot(SWS_onsets(valid_SWS_inds)/3600,LH_valid_smoo(SWS_inds(valid_SWS_inds)),'.','markersize',14,'linewidth',1.0,...
%     'color',.3*[.6 .6 .9]);
% title(fname(1:11));
% print(['C:\Users\Spike Sorting\Desktop\state_transition\' fname(1:11)],'-dpng');
% savefig(['C:\Users\Spike Sorting\Desktop\state_transition\' fname(1:11) '.fig']);
% close;

% state transitions detection:
window_len=[1 5 20]; % smoothing window length
for k=1:3 % 3 lengths for the choice of smoothing window length
SWS_onsets=[];
REM_onsets=[];
t0_=t_bins3sec(1); % initiation for the variable containing the times of SWS and REM initiation
t0=t_bins3sec(1); % initiation for the variable containing the times of SWS and REM initiation
...
state_transition(k)=sum(REM_onsets>light_off_t+20*60 & REM_onsets<light_on_t);
REM_onsets_{k,:}=REM_onsets(REM_onsets>light_off_t+20*60 & REM_onsets<light_on_t);
SWS_onsets_{k,:}=SWS_onsets(SWS_onsets>light_off_t+20*60 & SWS_onsets<light_on_t);
end
...
res(n).state_transitions=state_transition;
res(n).REM_onsets=REM_onsets_;
res(n).SWS_onsets=SWS_onsets_;
save('G:\Hamed\zf\P1\labled sleep\state_transition_result.mat','res','-nocompression')

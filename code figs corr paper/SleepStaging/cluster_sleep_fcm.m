function [bin_label,valid_inds]=cluster_sleep_fcm(feat, sleep_wake, Delta, Betta )
% removing the faulty bins (val=inf) or outlier value in features
m=median(feat);
d=20*iqr(feat);
comp1=repmat(m+d,length(feat),1);
comp2=repmat(m-d,length(feat),1);
in_range_ind=sum(feat<comp1 & feat>comp2,2)==size(feat,2);
valid_inds=~isinf(sum(feat,2)) & in_range_ind;

feat=feat(valid_inds,:);
sleep_wake=sleep_wake(valid_inds);
Delta=Delta(valid_inds);
Betta=Betta(valid_inds);

sleep_ind=logical(sleep_wake);
% first clustering, for SWS/nSWS
[~,SWS_nSWS_weights] = fcm(zscore(feat(sleep_ind,[1 3 4 5 6 7])),2);  % clustering only for sleep bins, fcm gives weights
[~,SWS_nSWS]=max(SWS_nSWS_weights); % turn weights to labels, by finding the index of the max weight for each sample bin
...
[~,REM_nREM_weights] = fcm(zscore(feat(sleep_ind,[2 3 4 5 7])),2);  % clustering only for sleep bins
[~,REM_nREM]=max(REM_nREM_weights);
% comparing Gamma across clusters to find the REM one
sleep_gamma=Betta(sleep_ind);

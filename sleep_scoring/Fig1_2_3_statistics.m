...
data=load('G:\Hamed\zf\P1\labled sleep\batch_results3with_dph'); % the file containing the batch result of all birds
sleep_vars=data.sleep_vars; % the variable containing the results
...
for bird_n=1:length(sleep_vars)
    % plotting
        i=bird_id(bird_n);
        err = sleep_vars(bird_n).iqr_LH/2;
        x=sleep_vars(bird_n).dph;
        y=sleep_vars(bird_n).median_LH;
    if i<=3 % for adults
        plot( x-600,y,'marker',bird_symbols{i},'color',col(i,:),'LineWidth',2); hold on
        errorbar(x-600,y, err, 'color',[col(i,:) .2]);
    else  % for juveniles
        plot(x,y,'marker',bird_symbols{i},'color',col(i,:),'LineWidth',2); hold on
        errorbar(x,y, err, 'color',[col(i,:) .2]);
    end
end
ylim([0 110]); xlim([40 210]) 
...
for bird_n=1:length(sleep_vars)
    % plotting
        i=bird_id(bird_n);
        var=sleep_vars(bird_n).local_wave_perSec_perChnl_mean; 
        err = std(var(~isnan(var)));
        x=sleep_vars(bird_n).dph;
        y=mean(var(~isnan(var)));
    if i<=3 % for adults
        plot( x-600,y,'marker',bird_symbols{i},'color',col(i,:),'LineWidth',2); hold on
        errorbar(x-600,y, err, 'color',[col(i,:) .2]);
    else  % for juveniles
...
mean_median_depth_juv=mean([sleep_vars(juv_inds).median_LH])
mean_median_depth_adult=mean([sleep_vars(adu_inds).median_LH])

iqr_median_depth_juv=iqr([sleep_vars(juv_inds).median_LH])
iqr_median_depth_adult=iqr([sleep_vars(adu_inds).median_LH])

all_local_wave_chnls_juv=[sleep_vars(juv_inds).local_wave_perSec_perChnl_mean];
mean_local_wave_juv=mean(all_local_wave_chnls_juv(~isnan(all_local_wave_chnls_juv)),'all')

all_local_wave_chnls_adu=[sleep_vars(adu_inds).local_wave_perSec_perChnl_mean];
...
[p,h]=ranksum( [sleep_vars(juv_inds).local_wave_perSec_perChnl_mean], [sleep_vars(adu_inds).local_wave_perSec_perChnl_mean] )
...
for bird_n=1:length(sleep_vars)
    ...
    if i<=3 % for adults
        ...
        subplot(1,3,1) %  SWS
        x=sleep_vars(bird_n).dph;
        plot( (x-age),sleep_vars(bird_n).LLRRLR_corr_SWS(1),'marker',bird_symbols{i},'color',col(i,:),...
            'LineWidth',1); hold on
        subplot(1,3,1) %  IS
        plot( (x-age)+30,sleep_vars(bird_n).LLRRLR_corr_IS(1),'marker',bird_symbols{i},'color',col(i,:),...
            'LineWidth',1); hold on
        subplot(1,3,1) %  REM
        plot( (x-age)+60,sleep_vars(bird_n).LLRRLR_corr_REM(1),'marker',bird_symbols{i},'color',col(i,:),...
            'LineWidth',1); hold on
        ...
        % RR corr
        subplot(1,3,2) %  SWS
        plot( 0+(x-age),sleep_vars(bird_n).LLRRLR_corr_SWS(2),'marker',bird_symbols{i},'color',col(i,:),...
            'LineWidth',1); hold on
        subplot(1,3,2) %  IS
        plot( 30+(x-age),sleep_vars(bird_n).LLRRLR_corr_IS(2),'marker',bird_symbols{i},'color',col(i,:),...
            'LineWidth',1); hold on
        subplot(1,3,2) %  REM
        plot( 60+(x-age),sleep_vars(bird_n).LLRRLR_corr_REM(2),'marker',bird_symbols{i},'color',col(i,:),...
            'LineWidth',1); hold on
        ...
        % RL corr
        subplot(1,3,3) %  SWS
        plot( 0+(x-age),sleep_vars(bird_n).LLRRLR_corr_SWS(3),'marker',bird_symbols{i},'color',col(i,:),...
            'LineWidth',1); hold on
        subplot(1,3,3) %  IS
        plot( 30+(x-age),sleep_vars(bird_n).LLRRLR_corr_IS(3),'marker',bird_symbols{i},'color',col(i,:),...
            'LineWidth',1); hold on
        subplot(1,3,3) %  REM
        plot( 60+(x-age),sleep_vars(bird_n).LLRRLR_corr_REM(3),'marker',bird_symbols{i},'color',col(i,:),...
            'LineWidth',1); hold on
        ylim([0 .95]); xlim(30*[-.2 2.8]);
        xticks([0 1 2]*30);  xticklabels({'SWS','IS','REM'});
...
for bird_n=1:length(sleep_vars)
    ...
    % corresponding bird, gives the age of the bird.    
        % LL corr
        subplot(1,3,1) %  SWS
        plot( 0+(x-age),sleep_vars(bird_n).LLRRLR_corr_SWS(1),'marker',bird_symbols{i},'color',col(i,:),...
            'LineWidth',1); hold on
        subplot(1,3,1) %  IS
        plot( 30+(x-age),sleep_vars(bird_n).LLRRLR_corr_IS(1),'marker',bird_symbols{i},'color',col(i,:),...
            'LineWidth',1); hold on
        subplot(1,3,1) %  REM
        plot( 60+(x-age),sleep_vars(bird_n).LLRRLR_corr_REM(1),'marker',bird_symbols{i},'color',col(i,:),...
            'LineWidth',1); hold on
        ylim([0 .95]); xlim(30*[-.2 2.8]); ylabel('corr coef')
...
for bird_n=1:length(sleep_vars)
    ...
    dos_loc_wave_corrs_juv(juv_n)=sleep_vars(juv_n).corr_local_wave_and_depth;
    juv_n=juv_n+1;
    elseif i<=3
    dos_loc_wave_corrs_adult(adult_n)=sleep_vars(adult_n).corr_local_wave_and_depth;
    adult_n=adult_n+1;
    end
end
...
%% corr of connectivity with age, adding the refgression line and statistical significance
% juvenule
bird_n=1:length(sleep_vars);
juv_id=(bird_id(bird_n)==4 | bird_id(bird_n)==5 | bird_id(bird_n)==6 | bird_id(bird_n)==8); % id of all juveniles
juv_n=1;
for bird_n=1:length(sleep_vars)
    ...
    juv_LLRRLR_corr_SWS(juv_n,:)=sleep_vars(bird_n).LLRRLR_corr_SWS;  
    juv_LLRRLR_corr_IS(juv_n,:)=sleep_vars(bird_n).LLRRLR_corr_IS;  
    juv_LLRRLR_corr_REM(juv_n,:)=sleep_vars(bird_n).LLRRLR_corr_REM;  
    juv_exp_day(juv_n,1)=sleep_vars(bird_n).dph-dph0; % age at day of recording
    juv_n=juv_n+1;
    end
end
...
for bird_n=1:length(sleep_vars)
    id=bird_id(bird_n);
    % only one of the parenthesis will be logical 1
    if (id==1  & sleep_vars(bird_n).bird(11)=='3') | id==2 | id==3 % if it is an adult
    dph0=780*(id==1)+686*(id==2)+780*(id==3)+50*(id==4)+50*(id==5)+51*(id==6)+52*(id==8); % depending on the bird ID, ...
    adult_LLRRLR_corr_SWS(adult_n,:)=sleep_vars(bird_n).LLRRLR_corr_SWS;  
    adult_LLRRLR_corr_IS(adult_n,:)=sleep_vars(bird_n).LLRRLR_corr_IS;  
    adult_LLRRLR_corr_REM(adult_n,:)=sleep_vars(bird_n).LLRRLR_corr_REM;  
    adult_exp_day(adult_n,1)=sleep_vars(bird_n).dph-dph0; % age at day of recording
    adult_n=adult_n+1;
    end
end
% LL, RR, and LR for SWS:
...
for bird_n=1:length(sleep_vars)
        i=bird_id(bird_n);
        local_wave_per_chnl = sleep_vars(bird_n).local_wave_perSec_perChnl_mean;
...
for bird_n=1:length(sleep_vars)
        i=bird_id(bird_n);
        sleep_depth_per_chnl = sleep_vars(bird_n).median_LH_per_chnl;

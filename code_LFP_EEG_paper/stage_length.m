function [bouts_Wake_len, bouts_SWS_len, bouts_IS_len, bouts_REM_len] = stage_length(bin_label,valid_bin_inds, t_feat)
...
for k=2:length(bin_label)
    if t_feat(adds(k))-t_feat(adds(k-1))>3.1 %  there has abeen an invalid bin between the last bin and the current one
        ...
        % start of a new bout
            bout_len=3; % 3 sec for the new bin
    else
        if strcmp(bin_label{k},bin_label{k-1})
              bout_len=bout_len+3; % 2 sec for each bin
        else
            switch bin_label{k-1}
                case 'Wake'
                    bouts_Wake_len=[bouts_Wake_len bout_len];

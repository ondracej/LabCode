function [data, timestamps, info] = load_open_ephys_data_adv(filename,downsamp,file_dev)
...
elseif strcmp(filetype, 'continuous')
    ...
    while ftell(fid) + RECORD_SIZE <= filesize/file_dev % at least one record remains
        ...
        if ~go_back_to_start_of_loop
            ...
            data(current_sample/downsamp+1:current_sample/downsamp+nsamples/downsamp) = ...
                downsample(block,downsamp);
    ...
    data = data(1:current_sample/downsamp);
    ...
    if version >= 0.1
        ...
        for record = 1:length(info.ts)
            ...
            timestamps(current_sample/downsamp+1:(current_sample+info.nsamples(record))/downsamp) = ...
                downsample(ts_interp(1:end-1),downsamp);

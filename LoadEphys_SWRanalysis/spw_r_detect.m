...
spws=zeros(2*fs_/5+1,N,length(spw_indx)); % initialization: empty spw matrix, length of spw templates is considered as 500ms
n=1;
...
subplot(3,1,1);
plot(time,signal_raw(:,k));  title('Raw signal'); ylabel('(\muV)'); xlim(plot_time);
subplot(3,1,2)
plot(time,RippSig), title('Ripple-filtered dange (50-300Hz)'); ylabel('(\muV)');    xlim(plot_time);
subplot(3,1,3)
...
Rip_indx=Rip_times+fs; % shifting 1 sec to the right place
...
for chnl=1:N
plot(time,signal_raw(:,chnl)-500*(chnl-1),'color',[150 200-100/N*chnl 100+100/N*chnl]); % color coded based on channel
hold on; 
if chnl==k % overlay sharp waves and ripple times on the plot
plot(spwT,signal_raw(spw_indx,chnl),'+r');  
plot(rippT,signal_raw(Rip_indx,chnl),'bv');
end
xlim(plot_time)

...
for n=1:nn-2 
subplot(nn,1,n)
plotredu(@plot,(t-t0)/60,X(:,n));  
ylabel({'EEG'; ['chnl' num2str(eeg_chnl(n))] });  xlim(plot_time/60);  xticks([]); ylim([-120 120])
if n==1
...
subplot(nn,1,n+2)
plotredu(@plot,(t_corr-t0)/60,eeg_eeg_corr,'c-','linewidth',1.5);    ylabel('temporal corr');   xlabel('Time (min)'); xlim(plot_time/60); ylim([0 1])
hold on; 
plotredu(@plot,(t_corr-t0)/60,eeg_dvr_corr,'m--','linewidth',1.5);    ylabel('temporal corr');   xlabel('Time (min)'); xlim(plot_time/60); ylim([0 1])
plotredu(@plot,(t_corr-t0)/60,dvr_dvr_corr,'k:','linewidth',1.5);    ylabel('temporal corr');   xlabel('Time (min)'); xlim(plot_time/60); ylim([0 1])

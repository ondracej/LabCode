...
k=1;  
while ceil(k*3*fs)<length(EEG)
EEG_part=EEG(floor((k-1)*3*fs)+1:ceil(k*3*fs),:); 
eeg_(:,:,k) = EEG_part(1:round(3*fs),:);
t_bin(k)=mean(time([floor((k-1)*3*fs)+1 ceil(k*3*fs)]));
k=k+1;
end
...
save(['G:\Hamed\zf\P1\labled sleep\' saving_name],'EEG3sec','t_bin','-v7.3','-nocompression');

...
[signal0, time, info] = load_open_ephys_data_chunked(filename);
...
wo = 50/(fs/2);  bw = wo/35; [b,a] = iirnotch(wo,bw); signal=filtfilt(b,a,signal0);
figure;
% Fig 1 (Raw)
subplot(4,1,1);plot(time,signal); title('Raw signal ' )
ylabel('(\muV)'); xlim(plot_time)

% filtering for sharp wave:
ShFilt = designfilt('bandpassiir','FilterOrder',4, 'HalfPowerFrequency1',2,'HalfPowerFrequency2',8, 'SampleRate',fs);
ShrpSig=filtfilt(ShFilt,signal);
% for ripples
RippFilt = designfilt('bandpassiir','FilterOrder',4, 'HalfPowerFrequency1',40,'HalfPowerFrequency2',280, 'SampleRate',fs);
RippSig=filtfilt(RippFilt,signal);

% Fig 1 (SW & R)
  subplot(4,1,2);plot(time,ShrpSig,'k');
title('Filtered 2-8Hz (LFP)' ); ylabel('(\muV)'); xlim(plot_time); 
  subplot(4,1,3);plot(time,RippSig,'r');
title('Filtered 80-300Hz (SWR)' ); ylabel('(\muV)'); xlabel('Time (Sec)')
xlim(plot_time); 

% Fig 1 ( ShR )
subplot(4,1,4);
plot(time,ShrpSig+RippSig,'b'); title('LFP + SWR' ); ylabel('(\muV)'); xlabel('Time (Sec)'); xlim(plot_time);
...
SpkSig=filtfilt(SpkFilt,signal);

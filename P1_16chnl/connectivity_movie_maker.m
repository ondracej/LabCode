function [] = connectivity_movie_maker(dir_add, t_off, t_on, fs, image_layout, xy, EEG, time,...
...
for t_vid_start=time(1):patch_time:time(end)-patch_time %time(end)-patch_time
    ...
    for t_int=t_vid_start:60:t_vid_start+patch_time-60 %  t_interval, taking one minute of data in a loop
        ...
        Cindex = ceil((c_int-min(c_int))/range(c_int(:))*length(cmap)+eps); % index for each entry, mapping ther values of ...
        % correlation coefficient to the range of numbers feom 1 to 256 for
        % colors

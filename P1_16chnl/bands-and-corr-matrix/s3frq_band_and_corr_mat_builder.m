...
for bin_center=t_bin
    ...
    tw = tukeywin(sum(bin_indices)); % Tukey window with coeff r = 0.5 to smooth the edges
    eeg_bin=eeg_part.*repmat(tw,1,size(EEG,2));
    % computing PSD for each frequency band, each channel, for this bin
    % of time:
    
    filt_eeg1=filtfilt(b1,a1,eeg_bin);
    band_power(1,:,k)=sum(pwelch(filt_eeg1,[],[],[],fs,'minhold'));
    filt_eeg2=filtfilt(b2,a2,eeg_bin);
    band_power(2,:,k)=sum(pwelch(filt_eeg2,[],[],[],fs,'minhold'));
    filt_eeg3=filtfilt(b3,a3,eeg_bin);
    band_power(3,:,k)=sum(pwelch(filt_eeg3,[],[],[],fs,'minhold'));
    filt_eeg4=filtfilt(b4,a4,eeg_bin);
    band_power(4,:,k)=sum(pwelch(filt_eeg4,[],[],[],fs,'minhold'));
    filt_eeg5=filtfilt(b5,a5,eeg_bin);
    band_power(5,:,k)=sum(pwelch(filt_eeg5,[],[],[],fs,'minhold'));
    filt_eeg6=filtfilt(b6,a6,eeg_bin);
    band_power(6,:,k)=sum(pwelch(filt_eeg6,[],[],[],fs,'minhold'));
    ...
    T_off=t_off(1)*3600+t_off(2)*60+t_off(3);
    T_on=t_on(1)*3600+t_on(2)*60+t_on(3);
    light_on(k)=bin_center<T_off | bin_center>T_on; % light status
    
    k=k+1;
    if rem(bin_center,3600)<bin_len-1-eps
        disp([num2str(round(bin_center/3600)) ' hour of EEG connectivity processed.']);
    end
end
end
